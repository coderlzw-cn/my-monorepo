...
/** 数据库诊断信息留在日志中；该函数只生成适合返回给客户端的中文文案。 */
const databaseMessage = (_key: string, fallback: string) => fallback;
...
export function resolveDatabaseError(exception: unknown): { status: HttpStatus; message: string } | null {
  if (exception instanceof Prisma.PrismaClientKnownRequestError) {
    ...
    if (!PRISMA_KNOWN_ERROR_CODES.has(errorCode)) {
      return { status: HttpStatus.INTERNAL_SERVER_ERROR, message: isProduction ? databaseMessage("OPERATION_FAILED", "数据库操作失败，请稍后重试") : exception.message };
    }
