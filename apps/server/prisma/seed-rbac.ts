...
async function main(): Promise<void> {
  ...
  try {
    const existingMockUserCount = await prisma.user.count({ where: { username: { startsWith: MOCK_USER_PREFIX } } });
    ...
    const result = await prisma.$transaction(
      async (transaction) => {
        ...
        const [superAdminSystemRole, userSystemRole] = await Promise.all([
          transaction.systemRole.upsert({
            where: { key: "SUPER_ADMIN" },
            create: { key: "SUPER_ADMIN", label: "超级管理员", description: "拥有系统全部权限", builtin: true },
            update: { label: "超级管理员", description: "拥有系统全部权限", builtin: true },
          }),
          transaction.systemRole.upsert({
            where: { key: "USER" },
            create: { key: "USER", label: "普通用户", description: "默认注册用户", builtin: true },
            update: { label: "普通用户", description: "默认注册用户", builtin: true },
          }),
        ]);
        ...
        for (const roleSeed of ROLE_SEEDS) {
          ...
          const role = await transaction.role.upsert({
            ...
            create: { key: roleSeed.key, label: roleSeed.label, description: roleSeed.description, level: roleSeed.level, parentId },
            update: { label: roleSeed.label, description: roleSeed.description, level: roleSeed.level, parentId },
          });
        ...
        await transaction.user.createMany({
          data: randomUsers.map((user, index) => ({
            ...
            systemRoleId: index < 25 ? superAdminSystemRole.id : userSystemRole.id,
          })),
        });

        const users = await transaction.user.findMany({ where: { username: { startsWith: MOCK_USER_PREFIX } }, select: { id: true }, orderBy: { username: "asc" } });
        const roles = await transaction.role.findMany({ where: { key: { in: ROLE_SEEDS.map((role) => role.key) } }, select: { id: true }, orderBy: { level: "desc" } });
        ...
        const assignments = users.flatMap((user, index) => {
          ...
          return [...roleIndexes].map((roleIndex) => ({ userId: user.id, businessRoleId: roles[roleIndex]!.id }));
        });

        await transaction.userBusinessRole.createMany({ data: assignments });

        return { users: users.length, roles: roles.length, assignments: assignments.length };
      },
