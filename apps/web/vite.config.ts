  ...
  return {
    plugins: [
      ...
      visualizer({
        ...
        sourcemap: false, // 生产环境不生成sourcemap，与 build.sourcemap 保持一致
      }),
      // 配置本地HTTPS开发环境
      basicSsl({
        name: "test", // 证书名称
        domains: ["*.example.com"], // 信任的域名（支持通配符）
        // 证书存储目录（import.meta.dirname 为 ESM 写法，等价于 CJS 的 __dirname）
        certDir: path.resolve(import.meta.dirname, ".devServer/cert"),
      }),
      // 配置HTML处理插件
      createHtmlPlugin({
        ...
        inject: {
          data: {
            title: env.VITE_APP_TITLE, // 注入HTML模板的数据（如标题）
          },
        },
      }),
      ViteImageOptimizer({}),
    ...
    server: {
      ...
      proxy: {
        "/api": {
          ...
          changeOrigin: true,
          // 参数命名为 urlPath，避免与顶部导入的 node:path 模块同名遮蔽
