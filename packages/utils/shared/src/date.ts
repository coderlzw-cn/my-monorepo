...
const RELATIVE_TIME_UNITS: readonly { unit: Intl.RelativeTimeFormatUnit; milliseconds: number }[] = [
...
export function formatDateTime(input: DateInput, options: FormatDateOptions = {}): string {
  ...
  try {
    const parts = getDateParts(date, { locale, ...(timeZone === undefined ? {} : { timeZone }) });
...
export function formatDateRange(start: DateInput, end: DateInput, options: FormatDateRangeOptions = {}): string {
  ...
  const endText = formatDateTime(endDate, { ...options, pattern: getRangeEndPattern(options.pattern, options.preset) });
...
export function formatDuration(duration: number, options: FormatDurationOptions = {}): string {
  ...
  const units = [
    { value: Math.floor(remaining / MILLISECONDS_PER_DAY), compact: "d", normal: "天" },
    { value: Math.floor((remaining % MILLISECONDS_PER_DAY) / MILLISECONDS_PER_HOUR), compact: "h", normal: "小时" },
    { value: Math.floor((remaining % MILLISECONDS_PER_HOUR) / MILLISECONDS_PER_MINUTE), compact: "m", normal: "分钟" },
    { value: Math.floor((remaining % MILLISECONDS_PER_MINUTE) / MILLISECONDS_PER_SECOND), compact: "s", normal: "秒" },
    { value: remaining % MILLISECONDS_PER_SECOND, compact: "ms", normal: "毫秒" },
  ];
...
export function isSameDate(first: DateInput, second: DateInput, timeZone?: string): boolean {
  ...
  try {
    const firstParts = getDateParts(firstDate, { locale: "en-US", ...(timeZone === undefined ? {} : { timeZone }) });

    const secondParts = getDateParts(secondDate, { locale: "en-US", ...(timeZone === undefined ? {} : { timeZone }) });
...
function getDateParts(date: Date, options: { locale: string; timeZone?: string }): DateParts {
