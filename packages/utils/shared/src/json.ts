...
export function parseJson<T = unknown>(text: string, options: ParseJsonOptions = {}): JsonParseResult<T> {
  ...
  try {
    ...
    return { success: false, error: fieldName === undefined ? normalizedError : new SyntaxError(`${fieldName} 不是有效的 JSON: ${normalizedError.message}`, { cause: error }) };
  }
}
...
export function formatJson(text: string, options: FormatJsonOptions = {}) {
  ...
  const formatted = stringifyJson(result.data, { format: outputFormat, space, sortKeys });
