...
export function formatNumber(value: NumberInput | null | undefined, options: FormatNumberOptions = {}): string {
  ...
  const formattedValue = new Intl.NumberFormat(locale, { minimumFractionDigits: precision, maximumFractionDigits: precision, useGrouping }).format(finalValue);
...
function resolveNumberValidationOptions(options: NumberValidationOptions): ResolvedNumberValidationOptions {
  ...
  return { minimum, maximum, integer, safeInteger, exclusiveMinimum, exclusiveMaximum };
}
